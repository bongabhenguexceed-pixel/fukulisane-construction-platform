...
export function AccordionItem({ defaultOpen = false, className, children, ...props }: any) {
  const [open, setOpen] = useState(defaultOpen)
  return (
    <AccordionItemContext.Provider value={{ open, toggle: () => setOpen(!open) }}>
      <div className={cn('border-b border-border last:border-0', className)} {...props}>{children}</div>
...
export function AccordionTrigger({ className, children, ...props }: HTMLAttributes<HTMLButtonElement>) {
  ...
  return (
    <button className={cn('flex w-full items-center justify-between px-4 py-3 text-sm font-medium transition-all hover:underline', className)} onClick={toggle} {...props}>
      {children}
...
export function AccordionContent({ className, children, ...props }: HTMLAttributes<HTMLDivElement>) {
  ...
  if (!open) return null
  return <div className={cn('px-4 pb-3 text-sm', className)} {...props}>{children}</div>
}